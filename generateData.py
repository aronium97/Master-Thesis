...
#task_duration = mu + 1 + np.ro1und(np.random.random([noOfUsers, noOfTasks]),5)
...
upload_speed = [30,40,50]#[20,30,50,40.99,30.1,20.1,19.9]
processing_speed = [50,60,55]#[12,10,2,8.1,8.9,9,9.1,12.1]
task_sizes = np.linspace(1,7,40)#, size=noOfTasks)#[1,3,7]# [1,5,3,2,4]
...
#        distances[i,j] = np.random.randint(0,1) #hs.haversine(locationOfUsers[i], locationOfTasks[j], unit=Unit.KILOMETERS)


# caluclate alpha k
alphakFileName = "data/" + str(noOfTasks) + "_auto_" + str(customName) + "alphak" + ".pkl"
if os.path.isfile(alphakFileName):
    print("alphak exists")
...
availabilitiesFileName = "data/" + str(noOfTasks) + "_auto_" + str(customName) + "availabilities" + ".pkl"
if os.path.isfile(availabilitiesFileName):
    print("availabilities exists")
    with open(availabilitiesFileName, 'rb') as f:
...
for i in range(0, noOfTasks):
    ...
    if not(counts == 1).all():
        raise ValueError("no strict ordering of users"  + str(t_processing[:,i] + t_upload[:,i]) + str(-1*t_processing - t_upload + mcsp_utility_without_revenue*revenuePerMbit))
