...
def print_hi(name):
    ...
    flname = "5m5auto"#"legit 10m20 soft braker"
    pickelFileName = "autoresults/" + flname
    with open(pickelFileName + ".pkl", 'rb') as f:
        data = pickle.load(f)

    newData = copy.deepcopy(data[0])
    noOfInstances = len(data)
    for iInstance in range(1, noOfInstances):
        data_i = data[iInstance]

        newData[0] = np.divide(newData[0]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[0]),(1+iInstance))
        newData[1] = np.divide(newData[1]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[1]),(1+iInstance))
        newData[2] = np.divide(newData[2]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[2]),(1+iInstance))

        newData[4] = np.divide(newData[4]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[4]),(1+iInstance))
        newData[5] = np.divide(newData[5]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[5]),(1+iInstance))
        newData[6] = np.divide(newData[6]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[6]),(1+iInstance))

        newData[9] = np.divide(newData[9]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[9]),(1+iInstance))
        newData[10] = np.divide(newData[10]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[10]),(1+iInstance))
        newData[11] = np.divide(newData[11]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[11]),(1+iInstance))

        newData[13] = np.divide(newData[13]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[13]),(1+iInstance))
        newData[14] = np.divide(newData[14]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[14]),(1+iInstance))
        newData[15] = np.divide(newData[15]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[15]),(1+iInstance))
        newData[16] = np.divide(newData[16]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[16]),(1+iInstance))
        newData[17] = np.divide(newData[17]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[17]),(1+iInstance))
        newData[18] = np.divide(newData[18]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[18]),(1+iInstance))
        newData[19] = np.divide(newData[19]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[19]),(1+iInstance))

        newData[22] = np.divide(newData[22]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[22]),(1+iInstance))

        newData[30] = np.divide(newData[30]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[30]),(1+iInstance))

        newData[58] = np.divide(newData[58]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[58]),(1+iInstance))
        newData[59] = np.divide(newData[59]*iInstance,(1+iInstance)) + np.divide(np.array(data_i[59]),(1+iInstance))
    ...
    if checkForStability:
        results = Parallel(n_jobs=noOfExperiments)(
            delayed(checkTheStability)(newData, iExperiment) for iExperiment in
    ...
    for iExperiment in expList:#range(noOfExperiments):#expList:#range(noOfExperiments):
        ...
        meanPessimalReward = newData[0][iExperiment]
        user_reward_expectation = newData[1][iExperiment]
        mcsp_reward_expectation = newData[2][iExperiment]

        meanOptimalReward = newData[4][iExperiment]
        meanOptimalGlobalReward = newData[5][iExperiment]
        meanPessimalGlobalReward = newData[6][iExperiment]

        task_duration = newData[9][iExperiment]
        stability = newData[10][iExperiment]
        noOfUnstableMatches = newData[11][iExperiment]

        noOfUsers = newData[23][iExperiment]
        noOfTasks = newData[27][iExperiment]

        globalReward = newData[13][iExperiment]
        estimated_task_duration = newData[14][iExperiment]
        estimated_task_reward = newData[15][iExperiment]
        noOfTimesVisited = newData[16][iExperiment]
        overDeadlineCounter = newData[17][iExperiment]
        noOfTimesChoosen = newData[18][iExperiment]
        freeSensingDone = newData[19][iExperiment]
        T = newData[20][iExperiment]
        deadline = newData[21][iExperiment]
        stableRegret = newData[22][iExperiment]

        globRew = newData[30][iExperiment]

        meanOptimalReward_MCSP = newData[58][iExperiment]
        rewardMeasurements_MCSP = newData[59][iExperiment]
    ...
    print("no of monte carlo iterations:" + str(len(newData[25][0])))
